...
;/¯¯¯¯ Reload_if_StrgS ¯¯ 190330180529 ¯¯ 30.03.2019 18:05:29 ¯¯\
/* recomandet using:
...
reload_if_StrgS(){
 	if(!A_PriorKey)
		return
	; Clipboard := A_PriorKey " " A_ThisHotkey
	; MsgBox,% Clipboard
 	if(A_ThisHotkey <> "~^s")
		return
	WinGetTitle, at, A ; v1 Style
	if(instr(at,A_ScriptName)){
		tip := A_ScriptName " will be reloaded in a second "
		tooltip,% tip "`n`n(" A_ThisFunc "~" A_LineNumber "~" RegExReplace(A_LineFile,".*\\"),1,20,9
		Sleep,100
		Reload
	}
}
;\____ Reload_if_StrgS __ 190330180538 __ 30.03.2019 18:05:38 __/
